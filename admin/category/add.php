...
$name = $description = $editor = $category = $business_platform = $active = '';
...
$editor = str_replace('"', '\\"', $editor) ;
  ...
  if($errorDescription == false && $errorName == false &&  $errorEditor == false){
    $sql = "INSERT INTO `category` (`name`, `parent_id`, `description`, `content`, `slug`, `active`, `business_platform_id`, `created_at`, `updated_at`) VALUES ( '".$name."', '".$category."', '".$description."', '".$editor."', '".createSlug($name)."', '1', '".$business_platform."', '".date("y-m-d h:m:s")."', '".date("y-m-d h:m:s")."')";
    execute($sql);
