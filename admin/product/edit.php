<?php
//unlink("../../public/img/2ae2d16bfba42f07a808783d6bfd75c2.jpg");
//die();
...
$priceSale = $dataEdit["price_sale"];
$dataImage = $dataEdit["thumb"];
$active = '1';
...
if(!empty($_FILES)){
    if (isset($_FILES['thumb'])) {
     ...
     if( $thumb["name"][0] == ""){
      ...
      //      die();
            $dataImage = $upload[0];
            unlink("../../public/img/".$dataEdit['thumb']);
  ...
  //var_dump($priceSale); die();
  $sqlUpload = "UPDATE `product` SET `name` = '".$name."', `category_id` = '".$category."', `description` = '".$description."', `price` = '".$price."', `price_sale` = '".$priceSale."', `content` = '".$content."', `slug` = '".createSlug($name)."', `active` = '1', `thumb` = '".$dataImage."', `admin_post_id` = '0', `view_model` = '0' WHERE `product`.`slug` = '".$dataEdit["slug"]."'";
   ...
   // $sql = "INSERT INTO `product` ( `name`, `category_id`, `description`, `price`, `price_sale`, `content`, `slug`, `active`, `thumb`, `admin_post_id`, `view_model`, `created_at`, `updated_at`) VALUES ( '".$name."', '".$category."', '".$description."', '".$price."', '".$priceSale."', '".$content."', '".createSlug($name)."', '1', '".$upload[0]."', '1', '1', '".date("y-m-d h:m:s")."', '".date("y-m-d h:m:s")."')";
    ...
    //var_dump($sql);
    //die;
    $state = "Sửa" .$dataEdit["name"]. "Thành công";
  }
         ...
         <div class="card-body">
            ...
            <div class="custom-file">
                ...
                <label class="custom-file-label" for="customFile">Chọn một ảnh đại diện sản phẩm</label>
                        ...
                        <img id="imgSrc" src="../../public/img/<?=$dataImage?>" style="margin-top : 30px" class="rounded" width="304" height="236">
        </div>
